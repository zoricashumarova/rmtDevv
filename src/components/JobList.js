...
const renderJobList = (whichList ='search') => {
    ...
    const jobItems = whichList === 'search' ? jobListSearchEl : jobListBookmarksEl;

    let jobs = [];
    if (whichList === 'search') {
        jobs = state.searchJobItems.slice(state.currentPage * RESULTS_PER_PAGE - RESULTS_PER_PAGE, state.currentPage * RESULTS_PER_PAGE);
    } else {
        jobs = state.bookmarkedJobItems;
    }
            ...
            <li class="job-item ${state.activeJobItem.id === jobItem.id ? 'job-item--active' : ""}">
                <a class="job-item__link" href="${jobItem.id}">
                    ...
                    <div class="job-item__right">
                        <i class="fa-solid fa-bookmark job-item__bookmark-icon ${state.bookmarkedJobItems.find((bookmarkedJobItem) => bookmarkedJobItem.id === jobItem.id) ? 'job-item__bookmark-icon--bookmarked' : ""}"></i>
